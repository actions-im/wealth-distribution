...
The expected-inheritance reallocation uses reported SCF expectations and estate intent with an explicit horizon and SSA mortality. It reallocates reported expectations rather than creating national wealth: recipient credits are offset by aggregate donor reserves, while conventional net worth remains unchanged. The public SCF does not link recipient to donor families, so the result is neither current legal ownership nor a guaranteed transfer or legal claim. The horizon is a visible scenario control, not observed transfer timing, and reported expected amounts are used without an invented probability haircut in the base scenario. Estate taxes, care costs, consumption, gifts, charity, siblings, and unobserved heirs are not modeled. No future return is added for an inherited asset already valued on the current owner's balance sheet.
